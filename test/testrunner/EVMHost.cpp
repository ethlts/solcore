	...
	m_evmVersion(_evmVersion)
{
	if (!m_vm)
	{
		...
		throw std::runtime_error("Unable to find evmone library");
	}
	...
	if (_evmVersion == langutil::EVMVersion::homestead())
		...
		throw std::runtime_error("Unsupported evm version");
...
evmc::Result EVMHost::call(evmc_message const& _message) noexcept
{
	...
	if (message.depth == 0)
	{
		message.gas -= message.kind == EVMC_CREATE ? GasCosts::txCreateGas : GasCosts::txGas;
		for (size_t i = 0; i < message.input_size; ++i)
			message.gas -= message.input_data[i] == 0 ? GasCosts::txDataZeroGas : GasCosts::txDataNonZeroGas(m_evmVersion);
		if (message.gas < 0)
		{
			evmc::Result result;
	...
	if (message.kind == EVMC_CREATE)
	{
		...
		auto encodeRlpInteger = [](int value) -> bytes {
			if (value == 0) {
				...
				solThrow(Exception, "Can only encode RLP numbers <= 0xffff");
			}
		};
		...
		solRequire(!accounts.contains(message.recipient), Exception, "Account cannot exist");
	...
	if (message.kind == EVMC_CREATE || message.kind == EVMC_CREATE2 || message.kind == EVMC_EOFCREATE)
	{
		int64_t codeDepositGas = static_cast<int64_t>(GasCosts::createDataGas * result.output_size);
...
) noexcept
{
	...
	if (_ignoresTrailingInput && !_inOut.empty())
	{
		...
		for (auto const& [input, output]: _inOut)
			solRequire(input.size() == _inOut.begin()->first.size(), Exception, "");
...
{
	solRequire(account_exists(_addr), Exception, "Account does not exist.");
	...
	static auto constexpr callKind = [](evmc_call_kind _kind) -> std::string
	{
		switch (_kind)
		{
			...
			case evmc_call_kind::EVMC_CREATE2:
				return "CREATE2";
			default:
				solThrow(Exception, "Invalid call kind.");
		}
	};
